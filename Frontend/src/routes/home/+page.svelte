<script>
    ...
    function openMine(){
        window.open("/employeePage","_self")
    }
    function openCust(){
...
<section class="header w-full h-24 bg-purple-100 m-0 absolute top-0 flex justify-between items-center pl-5 pr-5 border-b border-purple-500">
    ...
    <button class="logout w-16 h-8 bg-red-50 border  border-red-600  text-red-500 rounded-full hover:shadow-lg hover:shadow-red-500  hover:bg-red-500 hover:text-white" on:click={logout}>LOGOUT</button>
</section>
...
<section class="mnu p-6 grid grid-cols-3 gap-4 w-auto h-auto border border-t-0 border-purple-500 absolute top-24 ">
    ...
    <div class="tile" id="cust" on:click={openCust} on:keypress={openCust}>
        <div class="over"></div>
